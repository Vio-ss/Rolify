...
[x: string]: any;
  ...
  addcustomer() {
    if(this.haveadd){
      this.AddDialog('100ms', '500ms');
   }else{
     this.toastr.warning("You don't have access for Create")
   }
  }
  AddDialog(enteranimation: any, exitanimation: any) {
  ...
  EditDialog(enteranimation: any, exitanimation: any) {
    const popup = this.dialog.open(EditcustomerComponent, {
      enterAnimationDuration: enteranimation,
      exitAnimationDuration: exitanimation,
      width: '30%',
    });
